...
void Slic::generate_superpixels(IplImage *image, int step, int nc) {
	...
	{
		label_map.push_back(CvPoint(centers[i][3], centers[i][4]));
	}
}
...
void Slic::create_connectivity(IplImage *image) {
	...
	//vec2di new_clusters; //MOVED TO slic.h
	for (int i = 0; i < image->width; i++) {
		vector<int> nc;
		for (int j = 0; j < image->height; j++) {
			nc.push_back(-1);
			...
			if (new_clusters[i][j] == -1) {
				...
				if (count <= lims >> 2) {
					for (int c = 0; c < count; c++) {
						...
						//label_map[elements[c].x][elements[c].y] = new_clusters[elements[c].x][elements[c].y];
					}
					label -= 1;
				}
				label += 1;
...
void Slic::display_center_grid(IplImage *image, CvScalar colour) {
	for (int i = 0; i < (int)centers.size(); i++) {
		cvCircle(image, cvPoint(centers[i][3], centers[i][4]), 0, colour, 2);
	}
}
...
void Slic::colour_with_cluster_means(IplImage *image) {
	...
	mean_colours = colours;
}

/*
*
* List of nearest centers to input center (+ direction)
*
* Input : coord. of center, image
* Output: vector<CvPoint> with coord. of centers of nearest neighbors
*/
vector<CvPoint> Slic::find_neighbors(IplImage *image, int x_in, int y_in) {
	const int x = x_in;
	const int y = y_in;
	vector<CvPoint> neighbors; //left, up, right, down
	...
	for (int i = 0; i < 4; i++)
	{
		int x_local = x + dx4[i], y_local = y + dy4[i];
		while (clusters[x][y] == clusters[x_local][y_local])
		{
			if (dx4[i] == -1 && (x_local - 1) >= 0)
				x_local--;
			else
				if (dx4[i] == 1 && (x_local + 1) <= image->width)
					x_local++;
				else
					if (dy4[i] == -1 && (y_local - 1) >= 0)
						y_local--;
					else
						if (dy4[i] == 1 && (y_local + 1) <= image->height)
							y_local++;
						else
							break;
		}
		if (label_map[clusters[x_local][y_local]].x != x || label_map[clusters[x_local][y_local]].y != y)
			neighbors.push_back(label_map[clusters[x_local][y_local]]);
	}
